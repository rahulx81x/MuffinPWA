...
function seal(payload: unknown, secret: string) {
...
function unseal(token: string, secret: string) {
  const key = keyFromSecret(secret);
...
export function createSessionToken(
  session: Omit<SessionUser, never> & Record<string, unknown>
) {
...
export function readSession(event: NetlifyEvent): SessionUser | null {
  const secret = requireEnv('SESSION_SECRET');
...
export function requireSession(event: NetlifyEvent): SessionUser {
